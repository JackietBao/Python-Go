    ...
    actions: {
        ...
        async queryMyGroup() {
            ...
            this.groupList = [...res.data.map((item: any) => {
                const {id: uuid, title, demoData, desc, isSticky, createdAt, updatedAt, appId, logo, config} = item
                return {uuid, title, isEdit: false, demoData: demoData !== "" ? demoData.split("\n") : [], desc, appId, config, isSticky, logo, createdAt, updatedAt: new Date(updatedAt).getTime()}
            })]
