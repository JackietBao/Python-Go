...
const showNoData = computed(() => {
		return dataSources.value.length === 0 || (dataSources.value.length > 0 && appList.value.length === 0) || (dataSources.value.length > 0 && todayList.value.length === 0)
})
...
<template>
  <NScrollbar class="px-4">
    <div class="flex flex-col gap-3 text-sm">
        ...
        <ListItem v-if="stickyList.length && activeChatTab !== 'app'" :key="1000 + customKeyId" title="置顶" :data-sources="stickyList" @select="handleSelect" @delete="handleDelete" />

        <ListItem v-if="appList.length && activeChatTab === 'app'" :key="2000 + customKeyId" hideSticky title="应用" :data-sources="appList" @select="handleSelect" @delete="handleDelete" />

				<!-- 空数据 -->
				<template v-if="!appList.length && activeChatTab === 'app'">
					<div class="flex flex-col items-center mt-4 text-center text-neutral-300">
						<SvgIcon icon="ri:inbox-line" class="mb-2 text-3xl" />
						<span>{{ $t('common.noData') }}</span>
					</div>
				...
				<ListItem v-if="todayList.length  && activeChatTab !== 'app'" :key="3000 + customKeyId" title="今天" :data-sources="todayList" @select="handleSelect" @delete="handleDelete" />

				<ListItem v-if="otherList.length  && activeChatTab !== 'app'" :key="4000 + customKeyId" title="其他" :data-sources="otherList" @select="handleSelect" @delete="handleDelete" />

				<!-- 空数据 -->
				<template v-if="!todayList.length && !otherList.length && !stickyList.length && activeChatTab !== 'app'">
					<div class="flex flex-col items-center mt-4 text-center text-neutral-300">
						<SvgIcon icon="ri:inbox-line" class="mb-2 text-3xl" />
						<span>{{ $t('common.noData') }}</span>
					</div>
