...
<template>
  ...
  <div v-for="item of dataSources" :key="`${item.uuid}`">
    <a
      class="relative flex items-center dark:border-transparent  border-gray-200 gap-3 px-3 py-2.5 break-all border rounded-md cursor-pointer  hover:bg-gradient-to-l  dark:from-[#3076ED]  dark:to-[#54A2F5]  hover:dark:text-[#fff]  group "
      :class="[isActive(item.uuid) ? ' dark:text-[#fff] text-[#fff] bg-gradient-to-l from-[#3076ED] border-transparent to-[#54A2F5] bg-[#3144F1]  pr-20' : 'dark:bg-[#1E1E30]']"
    ...
    >
      ...
      <div v-if="isActive(item.uuid)" class="absolute z-10 flex visible right-1">
        ...
        <template v-else>
					...
					<button class="p-1" v-if="!hideSticky">
