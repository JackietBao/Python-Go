...
<template>
  <NModal :show="visible" :style="{ maxWidth: `${packageList.length > 4 ? 1200 : packageList.length * 250}px`, minWidth: isSmallMd ? '100%' : '1000px' }" :on-after-enter="openDialog" :on-after-leave="handleCloseDialog">
    <div class="p-4  rounded dark:bg-[#2D2D4F] bg-white max-h-4/5">
...
<!--              </template>-->
              ...
              <div>
                <p class="line-clamp-2 mb-4 dark:text-[#ccc]">{{ item.des }}</p>
                ...
                <div class="flex justify-between items-end mt-5">
                  ...
                  <NButton type="primary"   size="small" @click="handleSuccess(item)">
                    购买套餐
                  </NButton>
                </div>
              </div>
