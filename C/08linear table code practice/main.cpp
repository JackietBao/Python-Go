...
#define MaxSize 50//使用 MaxSize 来代表 50
typedef int ElemType;//让顺序表存储其他类型元素时，可以快速完成代码修改
typedef struct{
    ElemType data[MaxSize];//这是一个数组，用来存储顺序表的元素
    int length;//顺序表长度
}SqList;
...
bool ListDelete(SqList &L,int i,ElemType &k)
{
    //判断删除的元素的位置是否合法
    if(i<1 || i>L.length)
    {
...
void ListIn(Sq &L,int pos,Elem Inser)
{
